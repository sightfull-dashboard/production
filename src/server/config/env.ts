...
const normalizeDatabaseProvider = (value: string | undefined) => {
  const normalized = String(value ?? 'supabase').trim().toLowerCase();
  if (normalized === 'supabase') return normalized;
  throw new Error(`Unsupported DATABASE_PROVIDER: ${normalized}. SQLite runtime has been removed. Use supabase.`);
};
...
const sessionStoreDriver = normalizeDriver(process.env.SESSION_STORE_DRIVER, ['supabase'], 'supabase');
const authRateLimitStoreDriver = normalizeDriver(process.env.AUTH_RATE_LIMIT_STORE_DRIVER, ['memory', 'supabase'], 'supabase');
...
export const runtimeDatabaseShape = 'supabase-only';
