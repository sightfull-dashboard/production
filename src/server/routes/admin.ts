...
export function registerAdminRoutes({
  ...
  app.get("/api/admin/users", isSuperAdmin, (_req, res) => {
    ...
    res.json(users.map(mapAdminUserRow));
  });

  app.get("/api/admin/logs", isSuperAdmin, (_req, res) => {
  ...
  app.get("/api/admin/clients/:id/users", isSuperAdmin, (req, res) => {
  ...
  app.post("/api/admin/clients/:id/users", isSuperAdmin, (req, res) => {
    ...
    try {
      ...
      res.status(201).json(mapAdminUserRow(row));
    } catch (error: any) {
  ...
  app.patch("/api/admin/clients/:clientId/users/:userId", isSuperAdmin, (req, res) => {
    const existing = db.prepare("SELECT * FROM users WHERE id = ? AND client_id = ?").get(req.params.userId, req.params.clientId) as any;
    ...
    try {
      db.prepare("UPDATE users SET email = ?, password = ?, role = ?, name = ?, image = ?, is_verified = ? WHERE id = ?")
        .run(nextEmail, nextPassword, nextRole, nextName, nextImage, typeof req.body.is_verified === 'undefined' ? existing.is_verified : (req.body.is_verified ? 1 : 0), req.params.userId);
  ...
  app.delete("/api/admin/clients/:clientId/users/:userId", isSuperAdmin, (req, res) => {
  ...
  app.get("/api/admin/clients/:id/logs", isSuperAdmin, (req, res) => {
  ...
  app.get("/api/admin/clients/:id/payroll-logs", isSuperAdmin, async (req, res) => {
    const client = db.prepare("SELECT id, name FROM clients WHERE id = ?").get(req.params.id) as any;
  ...
  app.delete("/api/admin/clients/:id", isSuperAdmin, (req, res) => {
  ...
  app.post("/api/admin/users", isSuperAdmin, (req, res) => {
  ...
  app.patch("/api/admin/users/:id/verify", isSuperAdmin, (req, res) => {
  ...
  app.delete("/api/admin/users/:id", isSuperAdmin, (req, res) => {
  ...
  app.patch("/api/admin/users/:id", isSuperAdmin, (req, res) => {
    ...
    const nextVerified = typeof is_verified === 'undefined' ? existing.is_verified : (is_verified ? 1 : 0);
    ...
    try {
      db.prepare("UPDATE users SET email = ?, password = ?, role = ?, is_verified = ?, name = ?, image = ? WHERE id = ? AND client_id IS NULL")
        .run(nextEmail, nextPassword, nextRole, nextVerified, req.body.name ?? existing.name ?? null, req.body.image ?? existing.image ?? null, req.params.id);
  ...
  app.get("/api/admin/clients", isSuperAdmin, async (_req, res) => {
    ...
    try {
      ...
      const rows = await Promise.all((clients || []).map(async (client: any) => {
        const users = Number((db.prepare("SELECT COUNT(*) as count FROM users WHERE client_id = ?").get(client.id) as any)?.count || 0);
        const employees = Number((db.prepare("SELECT COUNT(*) as count FROM employees WHERE client_id = ? AND COALESCE(status, 'active') != 'offboarded'").get(client.id) as any)?.count || 0);
        const { count: filesCount } = await supabaseAdmin
          .from('files')
          .select('*', { count: 'exact', head: true })
          .eq('client_id', client.id);
        return serializeSupabaseClient(client, { users, employees, files: Number(filesCount) || 0 });
      }));
  ...
  app.patch("/api/admin/clients/:id", isSuperAdmin, async (req, res) => {
    ...
    const users = Number((db.prepare("SELECT COUNT(*) as count FROM users WHERE client_id = ?").get(req.params.id) as any)?.count || 0);
    const employees = Number((db.prepare("SELECT COUNT(*) as count FROM employees WHERE client_id = ? AND COALESCE(status, 'active') != 'offboarded'").get(req.params.id) as any)?.count || 0);
    const { count: filesCount } = await supabaseAdmin.from('files').select('*', { count: 'exact', head: true }).eq('client_id', req.params.id);

    logActivity(req, 'UPDATE_CLIENT', { clientId: req.params.id, name: updatePayload.name });
    return res.json(serializeSupabaseClient(updated, { users, employees, files: Number(filesCount) || 0 }));
  });
