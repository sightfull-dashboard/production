...
const PROTECTED_LEAVE_SHIFT_LABELS = ['absent', 'annual leave', 'sick leave', 'family leave', 'half day', 'unshifted'];
...
const getLeaveTypeFromShift = (db: any, shiftId: string | null | undefined) => {
  ...
  if (label === 'half day') return 'annual';
  return null;
};
  ...
  app.post('/api/employees/:id/restore', requireActiveTrial, requireUnlockedFeature('employee_records'), (req, res) => {
    ...
    try {
      ...
      logActivity(req, 'RESTORE_EMPLOYEE', {
        ...
        restored_emp_id: employee.emp_id,
